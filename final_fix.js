...
code = code.replace(
  /<th style="text-align: left; background: #0f172a;">Protocolos \/ Hormonas<\/th>/g,
...
code = code.replace(
  /<th>\$\{enc\}<\/th>/g,
  '<th style="font-size: 90%; font-weight: 500;">${window.tHeader ? window.tHeader(enc) : enc}</th>'
);

code = code.replace(
  /<th>Col \$\{i\+1\}<\/th>/g,
  '<th style="font-size: 90%; font-weight: 500;">Col ${i+1}</th>'
);
