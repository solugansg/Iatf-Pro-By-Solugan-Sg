...
window.exportarExcel = function() {
  ...
  // Guardar archivo
  const fileName = `Iatf Pro_Reporte_Integral_${pName.replace(/\s+/g, '_')}.xlsx`;
...
window.exportarPDF = function() {
  ...
  const opt = {
    ...
    filename:     `Iatf Pro_Reporte_${pName.replace(/\s+/g, '_')}.pdf`,
...
window.enviarWhatsApp = function() { 
  ...
  const perfil = JSON.parse(localStorage.getItem('iatfpro_perfil_consultor')) || {};
...
window.cargarDeHistorial = function(id) {
  ...
  if (confirm(`¿Deseas cargar el reporte de la finca "${record.finca}" realizado el ${record.fecha}?\n\nNota: Esto reemplazará los datos actuales en pantalla.`)) {
    localStorage.setItem('iatfpro_state', JSON.stringify(record.state));
    location.reload(); 
  }
};
